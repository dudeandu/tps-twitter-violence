// DATES:
    //"01/01/2015 00:00:00" >> 1420088400000
    //"12/31/2015 23:59:59" >> 1451624399000
    //"01/01/2016 00:00:00" >> 1451624400000
    //"12/31/2016 23:59:59" >> 1483246799000
    //"01/01/2017 00:00:00" >> 1483246800000
    //"12/31/2017 23:59:59" >> 1514782799000
    //"01/01/2018 00:00:00" >> 1514782800000
    //"12/31/2018 23:59:59" >> 1546318799000
    //"01/01/2019 00:00:00" >> 1546318800000
    //"12/31/2019 23:59:59" >> 1577854799000
    //"01/01/2020 00:00:00" >> 1577854800000
    //"12/31/2020 23:59:59" >> 1609477199000

        var filteredData;

        var dateFilter = {};

        var fromDate = 1577854800000;
        var toDate = 1609477199000;

        var fromHandle = 1577854800000, 
            toHandle = 1609477199000;
        ...
        var changedate = function(data) {
            ...
            if (data.eventNamespace == '.dateRangeSliderHandle1') {
                toHandle = data._value
            // console.log(toHandle)
            } else if (data.eventNamespace == '.dateRangeSliderHandle2') {
                fromHandle = data._value
            // console.log(fromHandle)

            }

            if (toHandle <= fromHandle) {
                fromDate = fromHandle
                toDate = toHandle
            } else {
                fromDate = toHandle
            ...
            dateFilter = {
                ...
                filterMilisec: [fromDate,toDate]
                }
            // console.log(dateFilter)

                    map.setFilter('circleTester',dateFilter.mapbox)
                    map.setFilter('circleTester_small',dateFilter.mapbox)
                    map.setFilter('heatTester',dateFilter.mapbox)
                // data.eventNamespace

                dateRangeArray = [new Date(toDate), new Date(fromDate)]


            // console.log(dateRangeArray[0].getTime(),dateRangeArray[1].getTime())

                $('#fromdateid').text(month[dateRangeArray[0].getMonth()] + " " + (dateRangeArray[0].getDate()<10?'0':'') + dateRangeArray[0].getDate() + ", " + dateRangeArray[0].getFullYear() );
                $('#todateid').text(month[dateRangeArray[1].getMonth()] + " " + (dateRangeArray[1].getDate()<10?'0':'') + dateRangeArray[1].getDate() + ", " + dateRangeArray[1].getFullYear() );
                
                // console.log(dateFilter.filterMilisec[0],dateFilter.filterMilisec[1])
                getTweetNumbers(dateFilter.filterMilisec[0],dateFilter.filterMilisec[1])
                d3.selectAll('.graphic-legend__legendtext').classed("activelegend", false).classed("filtered-out",false);

        }

        var getTweetNumbers = function(from, to) {
            ...
            // find the instances that fall between the filter dates
            for (var i = 0; i < jsonData.features.length; i++) {
                // console.log(jsonData.features[i].properties.militime, to, from)
                // if (jsonData.features[i].properties.event_type == 'exclude' || jsonData.features[i].properties.event_type == 'update') {
                if (jsonData.features[i].properties.event_type == 'exclude') {
                    // console.log(' excluded ')
                    excluded++;
                } else if (jsonData.features[i].properties.militime >= to 
                    && jsonData.features[i].properties.militime <= from ) {
                       
                        // if (jsonData.features[i].properties.Exclude != "y") {
                // Add the instances 
                    ...
                    // console.log(' ERROR ')
                    // console.log(jsonData.features[i], i)
                }
            }

            // replace the totals on the DOM
            $("#total-incidents-number").text(totalIncidents+1)
...
/*! jQRangeSlider 5.7.2 - 2019-08-14 - Copyright (C) Guillaume Gautreau 2012 - MIT and GPLv3 licenses.*/ ! function(a, b) {
    "use strict";
    ...
    function c(b, c, d, e) {
        this.label1 = b, this.label2 = c, this.type = d, this.options = e, this.handle1 = this.label1[this.type]("option", "handle"), this.handle2 = this.label2[this.type]("option", "handle"), this.cache = null, this.left = b, this.right = c, this.moving = !1, this.initialized = !1, this.updating = !1, this.Init = function() {
            ...
            if (this.CacheIfNecessary(), null !== this.cache) {
                ...
                this.label1[0].innerText === this.label2[0].innerText ? (this.PositionLabel(this.label1, (a.left + b.left) / 2, this.cache.label1), this.PositionLabel(this.label2, (a.left + b.left) / 2, this.cache.label2)) : (this.label1[d]("option", "isLeft") ? this.ConstraintPositions(a, b) : this.ConstraintPositions(b, a), this.PositionLabel(this.label1, a.left, this.cache.label1), this.PositionLabel(this.label2, b.left, this.cache.label2))
            }
...
function(a, b) {
    ...
    a.widget("ui.dateRangeSlider", a.ui.rangeSlider, {
        options: {
            bounds: {
                min: new Date(2020, 0, 1).valueOf(),
                max: new Date(2020, 11, 31, 23, 59, 59).valueOf()
            },
            defaultValues: {
                min: new Date(2020, 0, 1).valueOf(),
                max: new Date(2020, 11, 31, 23, 59, 59).valueOf()
...
function(a, b) {
    ...
    a.widget("ui.dateRangeSliderHandle", a.ui.rangeSliderHandle, {
        ...
        _getValueForPosition: function(a) {
            ...
            changedate(this)
            // totalsLoader(jsonData,fromDate,toDate)
            // console.log(a, this.options.bounds.min.valueOf(), this.options.bounds.max.valueOf())
